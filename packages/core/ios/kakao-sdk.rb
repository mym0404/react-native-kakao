module RNCKakaoSPM
  module Integration
    def apply_on_post_install(installer)
      ...
      installer.aggregate_targets.each do |aggregate|
        next unless aggregate.pod_targets.any? { |target| target.pod_name == 'RNCKakaoSocial' }

        aggregate.user_targets.each do |target|
          phase_name = '[RNCKakao] Copy Friend Resources'
          phase = target.shell_script_build_phases.find { |item| item.name == phase_name } || target.new_shell_script_build_phase(phase_name)
  ...
  def self.dependency(spec, versions, products)
    ...
    spec.pod_target_xcconfig = (spec.attributes_hash['pod_target_xcconfig'] || {}).merge(
      ...
      'FRAMEWORK_SEARCH_PATHS' => '$(inherited) "${PODS_CONFIGURATION_BUILD_DIR}/RNCKakaoCore"'
    )
    # Link shared static SDK objects once; every Kakao module already depends on Core.
    core = Pod::Spec.new { |dependency| dependency.name = 'RNCKakaoCore' }
    spm_dependency(
      core,
