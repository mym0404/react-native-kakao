...
:::info
Expo users must also meet the minimum version below. The Expo config plugin handles app configuration; configure the Kakao console manually.
:::

Kakao Android SDK 2.25.0 requires Android 6.0 (API 23)+. Set the app's `minSdkVersion` to at least 23, or higher if required by your React Native version.
