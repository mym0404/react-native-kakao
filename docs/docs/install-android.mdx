...
:::info
Expo를 사용해도 아래 최소 지원 버전을 확인해야 해요. 앱 설정은 Expo config plugin이 처리하므로 콘솔 설정만 직접 진행해요.
:::

Kakao Android SDK 2.25.0은 Android 6.0(API 23) 이상을 요구해요. 앱의 `minSdkVersion`을 23 이상으로 설정해야 해요. 사용하는 React Native 버전이 더 높은 값을 요구하면 그 조건을 따라야 해요.
