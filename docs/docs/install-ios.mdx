...
:::info
Expo에서는 앱 설정을 config plugin이 처리하므로 콘솔 설정만 직접 진행합니다. 설치 전에 [호환성 표](/docs/intro#compatibility)를 확인합니다.
:::

## SDK 설치 {#sdk-installation}

- `pod install`로 라이브러리를 설치합니다. podspec이 Kakao iOS SDK와 전이 의존성을 SPM에 등록하므로 Xcode에 패키지를 직접 추가하지 않습니다.
- Podfile의 `post_install`에서 React Native 기본 설정인 `react_native_post_install`을 호출합니다.
- 친구 선택 화면의 리소스 번들은 빌드할 때 자동으로 복사됩니다. `-clonedSourcePackagesDirPath`로 SPM 저장 경로를 변경했다면 같은 경로를 Xcode 빌드 설정 `KAKAO_SPM_SOURCE_PACKAGES_DIR`에도 전달합니다.
- Kakao SDK와 Alamofire의 버전은 `Podfile.lock`이 아닌 Xcode의 `Package.resolved`에서 확인합니다. CI에서도 이 파일을 유지합니다.
- 버전을 직접 지정하는 경우 [마이그레이션 안내](/docs/intro#migration)의 override 조건을 확인합니다.
